  ...
  return <div className="h-full overflow-y-auto bg-canvas px-4 py-5 text-content-primary sm:px-6 md:px-8 md:py-7">
    <div className="mx-auto max-w-5xl space-y-5 md:space-y-6">
          ...
          <div className="space-y-4 p-5 sm:p-6">
            <div className="rounded-lg border border-border bg-surface-subtle/60 p-4">
              ...
              <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
                <div className="min-w-0 flex-1"><Select aria-label="Agente para sugerir perguntas" value={selectedAgent} onChange={setSelectedAgent} placeholder="Selecione um agente" className="w-full min-h-[38px]" options={agents.map(agent => ({ value: agent.id, label: agent.name, description: agent.hasOpenaiKey ? 'Pronto para sugerir perguntas' : 'Sem chave de IA cadastrada' }))} /></div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div><label htmlFor="fact-category" className="mb-1.5 block text-xs font-semibold text-content-primary">Categoria</label><Select id="fact-category" aria-label="Categoria do fato" value={fact.category} onChange={value => setFact(previous => ({ ...previous, category: value as TrainingFact['category'] }))} className="w-full min-h-[38px]" options={categories} disabled={!canEdit} /></div>
      ...
      {step === 3 && <Card className="overflow-visible">
        ...
        <div className="space-y-4 p-5 sm:p-6">
          <div><label htmlFor="test-agent" className="mb-1.5 block text-xs font-semibold text-content-primary">Agente</label><Select id="test-agent" aria-label="Agente para teste" value={selectedAgent} onChange={setSelectedAgent} placeholder="Selecione um agente" className="w-full min-h-[38px]" options={agents.map(agent => ({ value: agent.id, label: agent.name, description: !agent.active_flow_version_id ? 'Sem fluxo publicado' : !agent.hasOpenaiKey ? 'Sem chave de IA cadastrada' : 'Pronto para teste' }))} /></div>
