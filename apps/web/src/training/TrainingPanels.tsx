  ...
  return <div className="space-y-4">
    {training.agents.length > 0 && <div className="rounded-lg border border-border bg-surface-elevated/60 p-4">
      ...
      <div className="flex flex-col gap-2 sm:flex-row">
        <div className="min-w-0 flex-1"><Select ariaLabel="Agente para sugerir perguntas" value={agentId} onChange={setAgentId} placeholder="Selecione um agente" className="min-h-[38px] w-full"
    ...
    <div className="grid gap-4 sm:grid-cols-[200px_1fr]">
      <div><label htmlFor="fact-category" className="mb-1.5 block text-xs font-semibold text-content">Categoria</label>
        <Select id="fact-category" ariaLabel="Categoria do fato" value={fact.category} onChange={value => setFact(previous => ({ ...previous, category: value as TrainingFact['category'] }))} className="min-h-[38px] w-full" options={factCategories} disabled={!training.canEdit} /></div>
...
export function TestPanel({ training }: { training: TrainingState }) {
  ...
  return <div className="space-y-4">
    <div><label htmlFor="test-agent" className="mb-1.5 block text-xs font-semibold text-content">Agente</label>
      <Select id="test-agent" ariaLabel="Agente para teste" value={agentId} onChange={setAgentId} placeholder="Selecione um agente" className="min-h-[38px] w-full"
