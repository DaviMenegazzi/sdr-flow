...
import { ArrowRight, Bot, MessageCircle, ShieldCheck, CircleStop, Workflow, Calendar, Database, AlertTriangle } from 'lucide-react';
...
export const FlowNode = memo(({ data, selected }: NodeProps<CanvasNode>) => {
  const { node, invalid, liveError } = data; const definition = catalog[node.type]; const ports = portsFor(node.type,node.config);
  ...
  return <div
    className={`flow-node ${selected ? 'selected' : ''} ${invalid ? 'invalid' : ''} ${liveError ? 'runtime-error' : ''}`}
    style={{ '--node-color': categoryColors[definition.category] } as React.CSSProperties}
    title={liveError ? `Falha na última execução real: ${liveError}` : undefined}
  >
    {liveError && (
      <div className="node-runtime-error-badge">
    ...
    {liveError && <div className="node-runtime-error-text">{liveError}</div>}
  </div>;
});
