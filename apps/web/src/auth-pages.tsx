...
export function LoginPage() {
  ...
  const submit=async(e:FormEvent)=>{e.preventDefault();if(!supabase){setError('Serviço de autenticação não configurado no cliente.');return;}setBusy(true);setError('');const result=await supabase.auth.signInWithPassword({email,password});setBusy(false);if(result?.error){setError(result.error.message==='Invalid login credentials'?'E-mail ou senha inválidos.':result.error.message);return;}const from=(location.state as {from?:string}|null)?.from;navigate(from?.startsWith('/')&&!from.startsWith('//')?from:'/flows/new',{replace:true});};
