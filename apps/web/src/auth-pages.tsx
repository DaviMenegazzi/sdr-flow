...
export function LoginPage() {
  ...
  if (session) return <Navigate to="/flows/new" replace />;
  const submit=async(e:FormEvent)=>{e.preventDefault();setBusy(true);setError('');const result=await supabase?.auth.signInWithPassword({email,password});setBusy(false);if(result?.error){setError('E-mail ou senha inválidos.');return;}const from=(location.state as {from?:string}|null)?.from;navigate(from?.startsWith('/')&&!from.startsWith('//')?from:'/flows/new',{replace:true});};
...
export function AuthCallback(){const navigate=useNavigate();useEffect(()=>{const timer=setTimeout(()=>navigate('/flows/new',{replace:true}),500);return()=>clearTimeout(timer)},[navigate]);return <AuthCard title="Confirmando conta"><p>Aguarde…</p></AuthCard>}
export function NotFoundPage(){return <AuthCard title="Página não encontrada"><p>A conta ou o recurso solicitado não está disponível.</p><Link to="/login">Voltar ao login</Link></AuthCard>}
