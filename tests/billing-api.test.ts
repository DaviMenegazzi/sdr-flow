...
async function runAs<T>(userId: string | null, sql: string, params: unknown[]): Promise<T[]> {
...
const SET_RETURNING = new Set(['billing_record_event', 'billing_claim_events', 'billing_reconciliation_report', 'get_organization_limits']);
function pgClient(userId: string | null) {
  ...
  const from = (table: string) => {
    ...
    const builder: any = {
      ...
      async execute() {
        ...
        try { return { data: await runAs(userId, sql, params), error: null }; } catch (error) { return { data: null, error: toError(error) }; }
      },
  ...
  return {
    ...
    async rpc(name: string, args: Record<string, unknown>) {
      ...
      try {
        const rows = await runAs<Record<string, unknown>>(userId, sql, values);
...
beforeAll(async () => {
  ...
  vi.spyOn(database, 'userDatabase').mockImplementation((_url, _key, token) => pgClient(token) as any);
  vi.spyOn(database, 'serviceDatabase').mockImplementation(() => pgClient(null) as any);
});
