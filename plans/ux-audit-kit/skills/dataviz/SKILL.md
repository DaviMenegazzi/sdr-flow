---
name: dataviz
description: Use this skill whenever you are about to create ANY chart, graph, plot, dashboard, or data visualization, in ANY output medium. Read it BEFORE writing the first line of chart code, choosing chart colors, building a stat tile / meter / KPI row, or laying out a dashboard. Teaches a design-system-agnostic method - a form heuristic, a color formula with a runnable validator, mark specs, and interaction rules. A validated default palette is documented in references/palette.md - swap that file's values for your brand's.
---
...
3. **VALIDATE the palette - run the script, don't reason about Delta E.**
   ...
   chart's own page, where it reads `data-palette` off `<body>` and logs a
   ...
   Fix anything that FAILs before continuing. Re-run for `--mode dark` with that
   mode's surface.
...
- **Run the validator before shipping any categorical palette.** CVD Delta E >= 8 is the
  target (OKLab x100); 6-8 is a floor that is legal ONLY with secondary encoding. A
...
| **Texture fill** | one directional hand-drawn fill, used at 45 / 135 degrees |
